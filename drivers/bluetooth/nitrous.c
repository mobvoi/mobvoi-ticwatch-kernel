	...
	struct nitrous_bt_lpm *lpm)
{
	...
	int len = 0;
	const __be32 *prop;
	u32 low_vol_level = 0;  /*regulator-min-microvolt*/
	u32 high_vol_level =0; /*regulator-max-microvolt*/
...
static int nitrous_initialize_gpio_state(struct platform_device *pdev)
   {
   int ret;
   struct pinctrl *pctrl;
   struct pinctrl_state *default_pctrl_state;
   struct pinctrl_state *sleep_pctrl_state;
   ...
   default_pctrl_state = pinctrl_lookup_state(pctrl, "default");
   if (IS_ERR(default_pctrl_state))
   {
   pr_err("default_pctrl get state failed\n");
   ret = PTR_ERR(default_pctrl_state);
   goto err;
   }
   sleep_pctrl_state = pinctrl_lookup_state(pctrl, "sleep");
   if (IS_ERR(sleep_pctrl_state))
   {
   pr_err("sleep_pctrl get state failed\n");
   ret = PTR_ERR(sleep_pctrl_state);
   goto err;
   }
   ret = pinctrl_select_state(pctrl, default_pctrl_state);
   if (ret)
   {
   pr_err("default_pctrl enable state failed\n");
   goto err;
   }
   return 0;
   err:
...
static int nitrous_probe(struct platform_device *pdev)
{
	...
	if (!lpm)
		return -ENOMEM;
	lpm->pdev = pdev;
	nitrous_initialize_gpio_state(pdev);
