...
static ssize_t zinitix_touch_calibration_show(struct device *dev,
		...
		char *p_i8_buf)
{
	...
	i32_ret = zinitix_i2c_pda2_set_page(client,
					    g_zinitix_ts->is_suspend,
					    ZINITIX_PDA2_PAGE_0);
	if (i32_ret < 0)
		goto exit_i2c_error;

	u8_rbuffer[0] = ZINITIX_HOST_CMD_CALIBRATION;
	i32_ret = zinitix_i2c_pda2_write(client, ZINITIX_PDA2_HOST_CMD_ADDR,
					 u8_rbuffer, 1);
	if (i32_ret < 0)
		goto exit_i2c_error;

	do {
		if (u8_rbuffer[0] == ZINITIX_HOST_CMD_NO_OP)
			break;

		msleep(1000);

		i32_ret = zinitix_i2c_pda2_read(client,
...
static ssize_t zinitix_i2c_pda_access_show(struct device *dev,
		struct device_attribute *attr,
		char *p_i8_buf)
{
	...
	mutex_lock(&g_zinitix_ts->lock);
	i32_ret = zinitix_i2c_pda_read(client,
				       g_u32_addr,
				       u8_rbuffer,
				       g_u32_length);
	mutex_unlock(&g_zinitix_ts->lock);
...
static ssize_t zinitix_i2c_pda_access_via_pda2_show(struct device *dev,
		struct device_attribute *attr,
		char *p_i8_buf)
{
	unsigned char u8_rbuffer[4];
...
static ssize_t zinitix_check_i2c_show(struct device *dev,
				      struct device_attribute *attr,
				      char *p_i8_buf)
{
	unsigned char u8_rbuffer[4];
	...
	i32_ret = handle_i2c_pda_read(g_zinitix_ts->client,
				      ZINITIX_CHK_I2C_CMD, u8_rbuffer, 4);
	if (i32_ret < 0)
		goto exit_i2c_error;
...
static ssize_t zinitix_hw_reset_show(struct device *dev,
				     struct device_attribute *attr,
				     char *p_i8_buf)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
...
static ssize_t zinitix_palm_status_show(struct device *dev,
					struct device_attribute *attr,
					char *p_i8_buf)
{
	unsigned short u16_len = 0;
	...
	zinitix_read_touchdata(u8_tp_status, u8_tp_buf);
...
static ssize_t zinitix_touch_lock_store(struct device *dev,
					struct device_attribute *attr,
					const char *p_i8_buf, size_t count)
{
	...
	case 1: /* Enable Touch lock */
		...
		i32_ret = zinitix_i2c_pda2_write(client,
						 ZINITIX_PDA2_HOST_CMD_ADDR,
						 u8_wbuffer,
						 1);
		if (i32_ret < 0)
...
static ssize_t zinitix_check_driver_version_show(struct device *dev,
		struct device_attribute *attr,
		char *p_i8_buf)
{
...
static ssize_t zinitix_check_fw_version_show(struct device *dev,
		struct device_attribute *attr,
		char *p_i8_buf)
{
	unsigned char u8_rbuffer[4];
	...
	if (i32_ret < 0)
		goto exit_i2c_error;
	i32_ret = zinitix_i2c_pda2_read(client, ZINITIX_PDA2_FW_VERSION_ADDR,
					u8_rbuffer, 4);
	if (i32_ret < 0)
		goto exit_i2c_error;
...
static ssize_t zinitix_check_panel_version_show(struct device *dev,
		struct device_attribute *attr,
		char *p_i8_buf)
{
	...
	if (i32_ret < 0)
		goto exit_i2c_error;
	i32_ret = zinitix_i2c_pda2_read(client,
					ZINITIX_PDA2_PANEL_VERSION_ADDR,
					u8_rbuffer, 8);
	if (i32_ret < 0)
		goto exit_i2c_error;
...
static ssize_t zinitix_fw_upgrade_store(struct device *dev,
					struct device_attribute *attr,
					const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
...
static ssize_t zinitix_fw_upgrade_show(struct device *dev,
                                       struct device_attribute *attr,
                                       char *p_i8_buf)
{
	return 0;
				       ...
				       char *p_i8_buf)
{
	...
	if ((g_u8_table_setting == 1) && (g_u8_table_init == 1))
		zinitix_mem_table_setting();

	if (g_u8_upgrade_type == 1) {
		i32_ret = zinitix_burn_fw(client);
		if (i32_ret < 0)
			goto exit_upgrade;

		i32_result = SUCCESS;
	} else if (g_u8_upgrade_type == 2) {
		i32_ret = zinitix_burn_comp(client);
		if (i32_ret < 0)
			goto exit_upgrade;
		...
		i32_ret = zinitix_load_test_fw(client);
		if (i32_ret < 0)
			goto exit_upgrade;
...
exit_upgrade:
	LOGD(LOG_DEBUG, "[touch]g_u8_zinitix_flag : %d", g_u8_zinitix_flag);
	g_u8_zinitix_flag &= ~ENG_MODE;
	g_u8_upgrade_type = 0;
...
#endif
static ssize_t zinitix_i2c_pda2_page_store(struct device *dev,
		struct device_attribute *attr,
		const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
	...
	i32_ret = zinitix_i2c_pda2_set_page(client, g_zinitix_ts->is_suspend, u8_page);
	if (i32_ret < 0)
		goto exit_set_error;
...
#if 1
#else
static ssize_t zinitix_mem_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
	...
	if (!zinitix_id_init(u8_type)) {
		LOGD(LOG_ERR, "[touch]Set Raydium id failed!\n");
		return count;
	}

	zinitix_mem_table_init(g_zinitix_ts->id);
	if (zinitix_mem_table_setting()) {
...
#endif
static ssize_t zinitix_i2c_raw_data_store(struct device *dev,
		struct device_attribute *attr,
		const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
	...
	u8_w_data[RAD_HOST_CMD_POS] = ZINITIX_HOST_CMD_NO_OP;
...
static ssize_t zinitix_i2c_raw_data_show(struct device *dev,
		struct device_attribute *attr,
		char *p_i8_buf)
{
	...
	int i32_retry = 0;
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	struct zinitix_ts_data *ts =
		(struct zinitix_ts_data *)i2c_get_clientdata(client);
	...
	for (i32_retry = 0; i32_retry < u8_retry_limit; i32_retry++) {
		...
		if ((u8_rbuffer[RAD_FT_CMD_POS] & ZINITIX_FT_UPDATE) ==
		    ZINITIX_FT_UPDATE)
			break;
	...
	u8_rbuffer[RAD_HOST_CMD_POS] = ZINITIX_HOST_CMD_NO_OP;
...
static ssize_t zinitix_i2c_pda_access_store(struct device *dev,
		struct device_attribute *attr,
		const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
	...
	if (temp_buf && u32_data_count) {
		...
		mutex_lock(&g_zinitix_ts->lock);
		i32_ret = zinitix_i2c_pda_write(client, g_u32_addr,
...
static ssize_t zinitix_i2c_pda_access_via_pda2_store(struct device *dev,
		struct device_attribute *attr,
		const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
...
static ssize_t zinitix_i2c_pda2_mode_store(struct device *dev,
		struct device_attribute *attr,
		const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
	...
	if (i32_ret < 0)
		return i32_ret;
	i32_ret = zinitix_i2c_mode_control(client, u8_mode);
	if (i32_ret < 0)
		return i32_ret;
...
static ssize_t zinitix_i2c_pda2_access_show(struct device *dev,
		struct device_attribute *attr,
		char *p_i8_buf)
{
	unsigned char u8_rbuffer[4];
	...
	mutex_lock(&g_zinitix_ts->lock);
	i32_ret = zinitix_i2c_pda2_read(client, g_u8_addr,
...
static ssize_t zinitix_i2c_pda2_access_store(struct device *dev,
		struct device_attribute *attr,
		const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
	...
	if (temp_buf && u32_data_count) {
		...
		mutex_lock(&g_zinitix_ts->lock);
		i32_ret = zinitix_i2c_pda2_write(client, g_u8_addr,
						 u8_w_data, g_u32_length);
		mutex_unlock(&g_zinitix_ts->lock);
		if (i32_ret < 0)
			goto exit_error;
...
static ssize_t zinitix_receive_fw_store(struct device *dev,
					struct device_attribute *attr,
					const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
...
static ssize_t zinitix_log_level_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
...
#ifdef RAD_SELFTEST
static ssize_t zinitix_reset_control_store(struct device *dev,
		struct device_attribute *attr,
		const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
...
static ssize_t zinitix_irq_state_show(struct device *dev,
				      struct device_attribute *attr,
				      char *p_i8_buf)
{
	unsigned int u32_irq_value;

	u32_irq_value = gpio_get_value(g_zinitix_ts->irq_gpio);
...
static ssize_t zinitix_flag_show(struct device *dev,
				 struct device_attribute *attr,
				 char *p_i8_buf)
{
	unsigned short u16_len = 0;

	snprintf(p_i8_buf, PAGE_SIZE, "%d", g_u8_zinitix_flag);
...
static ssize_t zinitix_flag_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *p_i8_buf, size_t count)
{
	int i32_ret = 0;
	...
	if (i32_ret < 0)
		return i32_ret;
	g_u8_zinitix_flag = u8_flag;
	return count;
}

static ssize_t zinitix_selftest_show(struct device *dev,
				     struct device_attribute *attr,
				     char *p_i8_buf)
{
	int i32_ret = SUCCESS;
	...
	i32_ret = zinitix_do_selftest(g_zinitix_ts);
 ...
 *  example:cat zinitix_ic_verion
 */
static DEVICE_ATTR(zinitix_touch_calibration, 0644,
		   zinitix_touch_calibration_show,
		   NULL);

/* check the i2c (R)
 *  example:cat zinitix_check_i2c
 */
static DEVICE_ATTR(zinitix_check_i2c, 0644,
		   zinitix_check_i2c_show,
		   NULL);

/* upgrade configurate and algo firmware from app.bin (W)
 *  example:echo "offset num_of_bin length *_app.bin [length *_app.bin]"
 *  > zinitix_fw_upgrade_mode
 */
static DEVICE_ATTR(zinitix_fw_upgrade, 0644,
		   zinitix_fw_upgrade_show,
		   zinitix_fw_upgrade_store);

/* change I2C communication mode (W)
 *  example:echo 1 > zinitix_i2c_pda2_mode ==> enable pda2 mode
 *        echo 0 > zinitix_i2c_pda2_mode ==> disable pda2 mode
 */
static DEVICE_ATTR(zinitix_i2c_pda2_mode, 0644,
		   NULL,
		   zinitix_i2c_pda2_mode_store);

/* I2C pda mode (R/W)
 *  example:    cat zinitix_i2c_pda_access ==> read pda address provided by the
 *		following cmd
 *		echo ADDRinHEX [DATAinHEX] > zinitix_i2c_pda_access ==> write
 *		pda address [data]
 */
static DEVICE_ATTR(zinitix_i2c_pda_access, 0644,
		   zinitix_i2c_pda_access_show,
		   zinitix_i2c_pda_access_store);
 ...
 *		pda address [data]
 */
static DEVICE_ATTR(zinitix_i2c_pda_access_via_pda2, 0644,
		   zinitix_i2c_pda_access_via_pda2_show,
		   zinitix_i2c_pda_access_via_pda2_store);

/* I2C pda2 mode (R/W)
 *  example:    cat zinitix_i2c_pda2_access ==> read pda2 address provided by
 *		the following cmd
 *		echo ADDRinHEX [DATAinHEX] > zinitix_i2c_pda2_access ==>
 *		write pda2 address [data]
 */
static DEVICE_ATTR(zinitix_i2c_pda2_access, 0644,
		   zinitix_i2c_pda2_access_show,
		   zinitix_i2c_pda2_access_store);

/* I2C pda2 mode page (W)
 *  example:    echo PAGEinHEX > zinitix_i2c_pda2_page ==> write pda2 page
 */
static DEVICE_ATTR(zinitix_i2c_pda2_page, 0644,
		   NULL,
		   zinitix_i2c_pda2_page_store);

/* I2C read/set FT raw data (R/W)
 *  example:    cat zinitix_i2c_raw_data ==> read raw data with specific length
 *		of corresponding type provided by the following cmd
 *              echo DataTypeinHEX RawDataLengthinHEX > zinitix_i2c_raw_data
 *		==> set raw data type and its length
 */
static DEVICE_ATTR(zinitix_i2c_raw_data, 0644,
		   zinitix_i2c_raw_data_show,
		   zinitix_i2c_raw_data_store);

/* Touch lock (W)
 *  example:    echo 1 > zinitix_i2c_touch_lock ==> enable touch lock
 *            echo 0 > zinitix_i2c_touch_lock ==> disable touch lock
 */
static DEVICE_ATTR(zinitix_i2c_touch_lock, 0644,
		   NULL,
		   zinitix_touch_lock_store);

/* Log level (W)
 *  example:    echo 1 > zinitix_log_level ==> modify log level
 */
static DEVICE_ATTR(zinitix_log_level, 0644,
		   NULL,
		   zinitix_log_level_store);
/* show the fw version (R)
 *  example:cat zinitix_fw_version
 */
static DEVICE_ATTR(zinitix_check_fw_version, 0644,
		   zinitix_check_fw_version_show,
		   NULL);

/* show the driver version (R)
 *  example:cat zinitix_check_driver_version
 */
static DEVICE_ATTR(zinitix_check_driver_version, 0644,
		   zinitix_check_driver_version_show,
		   NULL);
/* show the panel version (R)
 *  example:cat zinitix_panel_version
 */
static DEVICE_ATTR(zinitix_check_panel_version, 0644,
		   zinitix_check_panel_version_show,
		   NULL);

static DEVICE_ATTR(zinitix_hw_reset, 0644,
		   zinitix_hw_reset_show,
		   NULL);

static DEVICE_ATTR(zinitix_palm_status, 0644,
		   zinitix_palm_status_show,
		   NULL);

static DEVICE_ATTR(zinitix_receive_fw_control, 0644,
		   NULL,
		   zinitix_receive_fw_store);

static DEVICE_ATTR(zinitix_mem_setting, 0644,
		   NULL,
		   zinitix_mem_store);
 ...
 *  example:cat zinitix_int_flag
 */
static DEVICE_ATTR(zinitix_int_flag, 0644,
		   zinitix_flag_show,
		   zinitix_flag_store);

static DEVICE_ATTR(zinitix_reset_control, 0644,
		   NULL,
		   zinitix_reset_control_store);

static DEVICE_ATTR(zinitix_irq_state, 0644,
		   zinitix_irq_state_show,
		   NULL);
static DEVICE_ATTR(zinitix_do_selftest, 0644,
		   zinitix_selftest_show,
		   NULL);
#endif
/*add your attr in here*/
struct attribute *zinitix_attributes[] = {
	&dev_attr_zinitix_touch_calibration.attr,
	&dev_attr_zinitix_check_i2c.attr,
	&dev_attr_zinitix_i2c_pda2_mode.attr,
	&dev_attr_zinitix_i2c_pda_access.attr,
	&dev_attr_zinitix_i2c_pda_access_via_pda2.attr,
	&dev_attr_zinitix_i2c_pda2_access.attr,
	&dev_attr_zinitix_i2c_pda2_page.attr,
	&dev_attr_zinitix_i2c_raw_data.attr,
	&dev_attr_zinitix_i2c_touch_lock.attr,
	&dev_attr_zinitix_fw_upgrade.attr,
	&dev_attr_zinitix_check_fw_version.attr,
	&dev_attr_zinitix_check_panel_version.attr,
	&dev_attr_zinitix_hw_reset.attr,
	&dev_attr_zinitix_palm_status.attr,
	&dev_attr_zinitix_check_driver_version.attr,
	&dev_attr_zinitix_receive_fw_control.attr,
	&dev_attr_zinitix_mem_setting.attr,
	&dev_attr_zinitix_log_level.attr,
#ifdef RAD_SELFTEST
	&dev_attr_zinitix_irq_state.attr,
	&dev_attr_zinitix_int_flag.attr,
	&dev_attr_zinitix_reset_control.attr,
	&dev_attr_zinitix_do_selftest.attr,
#endif
	NULL
};
