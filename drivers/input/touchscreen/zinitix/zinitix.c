...
static bool get_self_rx_data(struct bt541_ts_info *info, u8 *buff, int skip_cnt)
{
	...
	if (read_raw_data(client, BT541_RAWDATA_REG,
			  (char *)(buff), 28) < 0) {
		zinitix_printk("Failed to read short data\n");
		info->work_state = NOTHING;
		enable_irq(info->irq);
		up(&info->work_lock);
...
static bool ts_read_coord(struct bt541_ts_info *info)
{
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		if (zinitix_bit_test(info->touch_info.event_flag, i)) {
			...
			if (read_data(info->client, BT541_POINT_STATUS_REG + 2 + ( i * 4),(u8 *)(&info->touch_info.coord[i]),
		...
		if (read_data(info->client, BT541_POINT_STATUS_REG,(u8 *)(&info->touch_info), sizeof(struct point_info)) < 0) {
			dev_err(&client->dev, "Failed to read point info\n");
...
static int zinitix_suspend_power_control(struct bt541_ts_info *data, int on)
{
	int rc;
	dev_info(&data->client->dev,"zinitix_suspend_power_control on=%d\n", on);
	if (!on){
...
retry_upgrade:
	...
	if (write_cmd(client, 0x00F8) != I2C_SUCCESS) {
		zinitix_printk("jump to upgrade sequence fail\n");
		goto fail_upgrade;
	}
...
//	cap->ic_fw_size = 44*1024;
	...
	if ((checkMode == NULL) &&(ts_check_need_upgrade(info, cap->fw_version,
		...
		if (ts_upgrade_firmware(info, m_pFirmware[m_FirmwareIdx],cap->ic_fw_size) == false)
			goto fail_init;
...
#if ESD_TIMER_INTERVAL
	if (write_reg(client, BT541_PERIODICAL_INTERRUPT_INTERVAL,SCAN_RATE_HZ * ESD_TIMER_INTERVAL) != I2C_SUCCESS)
		goto fail_init;
...
static irqreturn_t bt541_touch_work(int irq, void *data)
{
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
			...
			if (x > maxX || y > maxY) {
				dev_err(&client->dev,"Invalid coord %d : x=%d, y=%d\n", i, x, y);
				continue;
			}
...
//				info->touch_info.coord[i].minor_width = PALM_REJECT_WIDTH;
	...
	memcpy((char *)&info->reported_touch_info, (char *)&info->touch_info,sizeof(struct point_info));
	input_sync(info->input_dev);
...
static bool ts_set_touchmode(u16 value)
{
	...
	if (value == TOUCH_DND_MODE) {
		if (write_reg(misc_info->client, BT541_DND_N_COUNT,
			...
			printk(KERN_INFO "[zinitix_touch] TEST Mode : ""Fail to set BT541_DND_N_COUNT %d.\n", SEC_DND_N_COUNT);
		if (write_reg(misc_info->client, BT541_DND_U_COUNT,
			SEC_DND_U_COUNT) != I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : ""Fail to set BT541_DND_U_COUNT %d.\n", SEC_DND_U_COUNT);
		if (write_reg(misc_info->client, BT541_AFE_FREQUENCY,
			SEC_DND_FREQUENCY) != I2C_SUCCESS)
			printk(KERN_INFO "[zinitix_touch] TEST Mode : ""Fail to set BT541_AFE_FREQUENCY %d.\n", SEC_DND_FREQUENCY);
	}
...
static void get_module_vendor(void *device_data)
{
	...
	if (info->pdata->tsp_vendor1 > 0 && info->pdata->tsp_vendor2 > 0 ) {
		...
		dev_info(&info->client->dev,"%s: TSP_ID: %d[%d]%d[%d]\n", __func__,
...
static void run_preference_read(void *device_data)
{
	...
	for (i = 0; i < info->cap_info.x_node_num * info->cap_info.y_node_num; i++) {
		zinitix_printk(" TX pref_data[%d]: %d\n",i, (s16)raw_data->pref_data[i]);
...
static int zinitix_power_init(struct bt541_ts_info *data, bool on)
{
	...
	if (IS_ERR(data->vdd_v1)) {
		...
		dev_err(&data->client->dev,"Regulator get failed vdd v1 rc=%d\n", rc);
		return rc;
	}
	...
	if (regulator_count_voltages(data->vdd_v1) > 0) {
		...
		if (rc) {
			zinitix_printk("Regulator set_vtg v1 failed vdd rc=%d\n", rc);
			goto reg_vdd_v1_put;
		}
	}
...
static int zinitix_ts_pinctrl_init(struct bt541_ts_info *bt541_data)
{
	...
	if (IS_ERR_OR_NULL(bt541_data->ts_pinctrl)) {
		...
		dev_dbg(&bt541_data->client->dev,"Target does not use pinctrl %d\n", retval);
	...
	if (IS_ERR_OR_NULL(bt541_data->pinctrl_state_active)) {
		...
		dev_err(&bt541_data->client->dev,"Can not lookup %s pinstate %d\n",PINCTRL_STATE_ACTIVE, retval);
		goto err_pinctrl_lookup;
	}

	bt541_data->pinctrl_state_suspend = pinctrl_lookup_state(bt541_data->ts_pinctrl,PINCTRL_STATE_SUSPEND);
	...
	bt541_data->pinctrl_state_release = pinctrl_lookup_state(bt541_data->ts_pinctrl,PINCTRL_STATE_RELEASE);
	if (IS_ERR_OR_NULL(bt541_data->pinctrl_state_release)) {
		retval = PTR_ERR(bt541_data->pinctrl_state_release);
		dev_dbg(&bt541_data->client->dev,"Can not lookup %s pinstate %d\n",
...
{
	int err = 0;
	dev_info(&data->client->dev,"zinitix_hw_reset: on = %d, gpio_reset = %d\n", on, data->pdata->gpio_reset);
	if(on)
	{
		...
		*/
		...
		if (err) {
		dev_err(&data->client->dev,"unable to set direction for gpio ""[%d]\n", data->pdata->gpio_reset);
		}
...
static int resume_hw_reset( struct bt541_ts_info* data,bool on )
{
	...
	if(on)
	{
		...
		*/
		...
		mdelay(1);
		gpio_direction_output(data->pdata->gpio_reset, 0);
		mdelay(10)/*eric modify 20170109*/;
		gpio_direction_output(data->pdata->gpio_reset, 1);
		mdelay(1);
		//err = gpio_direction_input(data->pdata->gpio_reset);
		if (err) {
...
static void zinitix_setup_drm_notifier(struct bt541_ts_info *pdata)
{
	...
	zinitix_printk("setting up drm notifier success\n");
		...
		const struct i2c_device_id *i2c_id)
{
	...
	if (ret) {
		if (ret == -EPROBE_DEFER) {
			zinitix_printk("probe defer selected, rc=%d\n", ret);
			return ret;
		}
	}
	...
	ret = gpio_request(info->pdata->gpio_reset,"bt541_reset_gpio");
   	if (ret) {
   		dev_err(&info->client->dev,"reset gpio request failed");
	...
	zinitix_printk("KEY_WAKEUP/KEY_SLEEP capability register finish\n");
#endif
	...
	if (pdata->orientation & TOUCH_XY_SWAP) {
		input_set_abs_params(info->input_dev, ABS_MT_POSITION_Y,info->cap_info.MinX,
			info->cap_info.MaxX + ABS_PT_OFFSET,0, 0);
		input_set_abs_params(info->input_dev, ABS_MT_POSITION_X,info->cap_info.MinY,
			info->cap_info.MaxY + ABS_PT_OFFSET,0, 0);
	} else {
		input_set_abs_params(info->input_dev, ABS_MT_POSITION_X,info->cap_info.MinX,
			info->cap_info.MaxX + ABS_PT_OFFSET,0, 0);
		input_set_abs_params(info->input_dev, ABS_MT_POSITION_Y,info->cap_info.MinY,
			info->cap_info.MaxY + ABS_PT_OFFSET,0, 0);
	}
	...
	if (ret)
		dev_err(&info->client->dev,"Unable to register fb_notifier: %d\n",ret);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
static int bt541_ts_remove(struct i2c_client *client)
{
	...
	if (info->ts_pinctrl) {
		if (IS_ERR_OR_NULL(info->pinctrl_state_release)) {
			devm_pinctrl_put(info->ts_pinctrl);info->ts_pinctrl = NULL;
		} else {
