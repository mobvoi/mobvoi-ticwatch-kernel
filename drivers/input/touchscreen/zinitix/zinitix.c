...
{
	struct bt541_ts_info *info = container_of(work, struct bt541_ts_info, tmr_work);
	struct i2c_client *client = info->client;
...
retry_fatal:
	...
	if (write_reg(info->client, 0xc000, 0x0001) != I2C_SUCCESS) {
		zinitix_err("power sequence error (vendor cmd enable)\n");
	...
	if (write_cmd(info->client, 0xc004) != I2C_SUCCESS) {
		zinitix_err("power sequence error (intn clear)\n");
		goto fail_check_hwid;
	}
	udelay(10);

	if (write_reg(info->client, 0xc002, 0x0001) != I2C_SUCCESS) {
		zinitix_err("power sequence error (nvm init)\n");
		goto fail_check_hwid;
	}
	msleep(20);
	...
	for (i = 0; i < 80; i += TC_SECTOR_SZ) {
		if (read_firmware_data(info->client,BT541_READ_FLASH,(u8 *)&check_data[i],
				       TC_SECTOR_SZ) < 0) {
			zinitix_err(
...
retry_upgrade:
	...
	for (flash_addr = 0; flash_addr < size;) {
		for (i = 0; i < page_sz / TC_SECTOR_SZ; i++) {
			if (write_data(client, BT541_WRITE_FLASH,
				...
				zinitix_info("error : write zinitix tc firmare\n");
				goto fail_upgrade;
			}
	...
	for (flash_addr = 0; flash_addr < size;) {
		for (i = 0; i < page_sz / TC_SECTOR_SZ; i++) {
			if (read_firmware_data(client, BT541_READ_FLASH,
			     (u8 *)&verify_data[flash_addr],TC_SECTOR_SZ) < 0) {
				zinitix_err("Failed to read firmare\n");
				goto fail_upgrade;
			}
...
fail_upgrade:
	...
	if (retry_cnt++ < INIT_RETRY_CNT) {
		zinitix_err("upgrade failed : so retry... (%d)\n", retry_cnt);
		goto retry_upgrade;
	}
...
static bool ts_hw_calibration(struct bt541_ts_info *info)
{
	...
	do {
		...
		if (read_data(client,BT541_EEPROM_INFO_REG,(u8 *)&chip_eeprom_info, 2) < 0)
			return false;
		...
		if (time_out++ == 4) {
			...
			msleep(20);
			write_cmd(client, BT541_CLEAR_INT_STATUS_CMD);
			zinitix_err("h/w calibration retry timeout.\n");
		}
	...
	if (write_reg(client,BT541_TOUCH_MODE, TOUCH_POINT_MODE) != I2C_SUCCESS)
		return false;

	if (info->cap_info.ic_int_mask != 0) {
		if (write_reg(client,BT541_INT_ENABLE_FLAG,info->cap_info.ic_int_mask) != I2C_SUCCESS)
			return false;
	}
...
static bool init_touch(struct bt541_ts_info *info)
{
	...
	checkMode = strnstr(saved_command_line, productionMode,strlen(saved_command_line));
retry_init:
	for (i = 0; i < INIT_RETRY_CNT; i++) {
		if (read_data(client, BT541_EEPROM_INFO_REG,(u8 *)&chip_eeprom_info, 2) < 0) {
			zinitix_err("Failed to read eeprom info(%d)\n", i);
			msleep(20);
			continue;
		} else
			break;
	...
	for (i = 0; i < INIT_RETRY_CNT; i++) {
		if (read_data(client, BT541_CHECKSUM_RESULT,(u8 *)&chip_check_sum, 2) < 0) {
			msleep(20);
			continue;
		}
#if defined(TSP_VERBOSE_DEBUG)
	...
	if (read_data(client, BT541_CHIP_REVISION,(u8 *)&cap->ic_revision, 2) < 0) {
		zinitix_info("failed to read chip revision\n");
		goto fail_init;
	}
	...
	if (read_data(client, BT541_BUTTON_SENSITIVITY,(u8 *)&cap->key_threshold, 2) < 0)
		goto fail_init;

	if (read_data(client, BT541_DUMMY_BUTTON_SENSITIVITY,(u8 *)&cap->dummy_threshold, 2) < 0)
		goto fail_init;

	if (read_data(client, BT541_TOTAL_NUMBER_OF_X,(u8 *)&cap->x_node_num, 2) < 0)
		goto fail_init;

	if (read_data(client, BT541_TOTAL_NUMBER_OF_Y,(u8 *)&cap->y_node_num, 2) < 0)
		goto fail_init;
	...
	if (read_data(client, BT541_AFE_FREQUENCY,(u8 *)&cap->afe_frequency, 2) < 0)
		goto fail_init;
	...
	zinitix_info("fw version: 0x%x, 0x%x, 0x%x, hw_id: 0x%x\n",
			cap->fw_version, cap->fw_minor_version,cap->reg_data_version, cap->hw_id);
#if TOUCH_ONESHOT_UPGRADE
	if ((checkMode == NULL)
		&& (ts_check_need_upgrade(info, cap->fw_version,
						cap->fw_minor_version,cap->reg_data_version,cap->hw_id) == true)
	...
	/* initialize */
	if (write_reg(client, BT541_X_RESOLUTION,(u16) pdata->x_resolution) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client, BT541_Y_RESOLUTION,(u16) pdata->y_resolution) != I2C_SUCCESS)
		goto fail_init;
	...
	if (write_reg(client, BT541_BUTTON_SUPPORTED_NUM,(u16) cap->button_num) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client, BT541_SUPPORTED_FINGER_NUM,(u16) MAX_SUPPORTED_FINGER_NUM) != I2C_SUCCESS)
		goto fail_init;
	...
	if (write_reg(client, BT541_INITIAL_TOUCH_MODE,TOUCH_POINT_MODE) != I2C_SUCCESS)
		goto fail_init;
	...
	if (write_reg(client, BT541_INT_ENABLE_FLAG,cap->ic_int_mask) != I2C_SUCCESS)
		goto fail_init;
	...
	if (info->touch_mode != TOUCH_POINT_MODE) {	/* Test Mode */
		if (write_reg(client, BT541_DELAY_RAW_FOR_HOST,RAWDATA_DELAY_FOR_HOST) != I2C_SUCCESS) {
...
#if ESD_TIMER_INTERVAL
	if (write_reg(client, BT541_PERIODICAL_INT_INTERVAL,SCAN_RATE_HZ * ESD_TIMER_INTERVAL) != I2C_SUCCESS)
		goto fail_init;
...
static bool mini_init_touch(struct bt541_ts_info *info)
{
	...
	if (write_reg(client, BT541_BUTTON_SUPPORTED_NUM,(u16) info->cap_info.button_num) != I2C_SUCCESS)
		goto fail_mini_init;
	...
	if (info->touch_mode != TOUCH_POINT_MODE) {
		if (write_reg(client, BT541_DELAY_RAW_FOR_HOST,RAWDATA_DELAY_FOR_HOST) != I2C_SUCCESS) {
			zinitix_err("Failed to set BT541_DELAY_RAW_FOR_HOST\n");
...
static irqreturn_t bt541_touch_work(int irq, void *data)
{
	...
	if (zinitix_bit_test(info->touch_info.status, BIT_ICON_EVENT)) {
		if (read_data(info->client, BT541_ICON_STATUS_REG,(u8 *) (&info->icon_event_reg), 2) < 0) {
	...
	if (reported == true || !zinitix_bit_test(info->touch_info.status, BIT_PT_EXIST)) {
		for (i = 0; i < info->cap_info.multi_fingers; i++) {
...
static bool ts_set_touchmode(u16 value)
{
	...
	if (value == TOUCH_DND_MODE) {
		if (write_reg(misc_info->client, BT541_DND_N_COUNT,SEC_DND_N_COUNT) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to set BT541_DND_N_COUNT %d.\n",SEC_DND_N_COUNT);
		if (write_reg(misc_info->client, BT541_DND_U_COUNT,SEC_DND_U_COUNT) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to set BT541_DND_U_COUNT %d.\n",SEC_DND_U_COUNT);
		if (write_reg(misc_info->client, BT541_AFE_FREQUENCY,SEC_DND_FREQUENCY) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to set BT541_AFE_FREQUENCY %d.\n",SEC_DND_FREQUENCY);
	}
	if (value == TOUCH_PDND_MODE) {
		if (write_reg(misc_info->client, BT541_DND_N_COUNT,SEC_PDND_N_COUNT) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to set BT541_DND_N_COUNT %d.\n",SEC_PDND_N_COUNT);
		if (write_reg(misc_info->client, BT541_DND_U_COUNT,SEC_PDND_U_COUNT) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to set BT541_DND_U_COUNT %d.\n",SEC_PDND_U_COUNT);
	...
	if ((value == TOUCH_CHECK_SELF_RX_MODE) || (value == TOUCH_CHECK_SELF_TX_MODE)) {
		...
		if (write_reg(misc_info->client, BT541_DND_U_COUNT,SEC_SELF_U_COUNT) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to set BT541_DND_U_COUNT %d.\n",SEC_PDND_U_COUNT);
		if (write_reg(misc_info->client, BT541_AFE_FREQUENCY,SEC_SELF_FREQUENCY) != I2C_SUCCESS)
			zinitix_info("TEST Mode:Fail to set BT541_AFE_FREQUENCY %d.\n",SEC_PDND_FREQUENCY);
	} else if (misc_info->touch_mode == TOUCH_DND_MODE
		   || misc_info->touch_mode == TOUCH_PDND_MODE) {
		if (write_reg(misc_info->client, BT541_DND_N_COUNT,misc_info->cap_info.N_cnt) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to reset BT541_AFE_FREQUENCY %d.\n",misc_info->cap_info.N_cnt);
		if (write_reg(misc_info->client, BT541_DND_U_COUNT,misc_info->cap_info.u_cnt) != I2C_SUCCESS)
			zinitix_info("TEST Mode: Fail to reset BT541_DND_U_COUNT %d.\n",misc_info->cap_info.u_cnt);
		if (write_reg(misc_info->client, BT541_AFE_FREQUENCY,misc_info->cap_info.afe_frequency)!= I2C_SUCCESS)
	...
	if (misc_info->touch_mode != TOUCH_POINT_MODE) {
		if (write_reg(misc_info->client, BT541_DELAY_RAW_FOR_HOST,RAWDATA_DELAY_FOR_HOST) != I2C_SUCCESS)
			zinitix_info("Fail to set BT541_DELAY_RAW_FOR_HOST.\n");
	}

	if (write_reg(misc_info->client, BT541_TOUCH_MODE,misc_info->touch_mode) != I2C_SUCCESS)
...
static int ts_upgrade_sequence(const u8 *firmware_data)
{
	...
	zinitix_info("start upgrade firmware\n");
	if (ts_upgrade_firmware(misc_info,firmware_data,misc_info->cap_info.ic_fw_size) == false) {
		enable_irq(misc_info->irq);
		misc_info->work_state = NOTHING;
		up(&misc_info->work_lock);
		return -EPERM;
...
static void fw_update(void *device_data)
{
	...
	case UMS:
		...
		ret = request_firmware(&fw_entry, firmware_name,&info->input_dev->dev);
		if (ret) {
...
static void get_threshold(void *device_data)
{
	...
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff),"%d", info->cap_info.threshold);
	set_cmd_result(info, finfo->cmd_buff,strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
}
...
static void get_chip_vendor(void *device_data)
{
	...
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff),"%s", BT541_VENDOR_NAME);
	set_cmd_result(info, finfo->cmd_buff,strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
}
...
static void get_y_num(void *device_data)
{
	...
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff),"%u", info->cap_info.y_node_num);
	set_cmd_result(info, finfo->cmd_buff,
		       strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
}
...
static void get_reference(void *device_data)
{
	...
	if (x_node < 0 || x_node >= info->cap_info.x_node_num ||
		...
		set_cmd_result(info, finfo->cmd_buff,strnlen(finfo->cmd_buff,
	...
	for (i = 0; i < info->cap_info.x_node_num; i++) {
		for (j = 0; j < info->cap_info.y_node_num; j++) {
			...
			if (raw_data->delta_data[i * info->cap_info.y_node_num + j] > max)
				max =
				    raw_data->delta_data[i *info->cap_info.y_node_num + j];
	...
	if (cur && cmd_found) {
		...
		do {
			if (*cur == delim || cur - buf == len) {
				...
				*(buff + strlen(buff)) = '\0';
				ret = kstrtou8(buff, 0,&(finfo->cmd_param[param_cnt]));
				if (ret)
					continue;
				start = cur + 1;
				memset(buff, 0x00, ARRAY_SIZE(buff));
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 41, "%d %d %d %d", cap->dummy_threshold,
			cap->key_threshold, cap->key_threshold,cap->dummy_threshold);
#endif
}
			       ...
			       unsigned int cmd, unsigned long arg)
{
	...
	case TOUCH_IOCTL_VARIFY_UPGRADE_DATA:
		...
		if (copy_from_user
		    (m_pFirmware[m_FirmwareIdx], argp,misc_info->cap_info.ic_fw_size))
			return -EPERM;

		version =(u16) (m_pFirmware[m_FirmwareIdx][52] |
	...
	case TOUCH_IOCTL_HW_CALIBRAION:
		...
		if (write_reg(misc_info->client,BT541_TOUCH_MODE, mode) != I2C_SUCCESS) {
...
fail_hw_cal:
	...
	case TOUCH_IOCTL_GET_REG:
		...
		if (read_data(misc_info->client, reg_ioctl.addr,(u8 *)&val, 2) < 0)
			ret = -EPERM;
	...
	case TOUCH_IOCTL_DONOT_TOUCH_EVENT:
		...
		misc_info->work_state = SET_MODE;
		if (write_reg(misc_info->client, BT541_INT_ENABLE_FLAG, 0) !=I2C_SUCCESS)
			ret = -EPERM;
	...
	case TOUCH_IOCTL_SEND_SAVE_STATUS:
		...
		if (write_cmd(misc_info->client, BT541_SAVE_STATUS_CMD) != I2C_SUCCESS)
			ret = -EPERM;
...
static int zinitix_init_gpio(struct bt541_ts_info *data, bool on)
{
	...
	if (on) {
		if (gpio_is_valid(data->pdata->gpio_int)) {
			err = gpio_request(data->pdata->gpio_int,"bt541_irq_gpio");
			if (err) {
			     ...
			     struct bt541_ts_platform_data *pdata)
{
	...
	zinitix_err("gpio_int: %d\n",pdata->gpio_int);

	ret =of_property_read_u32(np, "zinitix,tsp_vdd_supply_type",&pdata->tsp_supply_type);
	if (ret < 0) {
		zinitix_err("failed to read property tsp_vdd_supply_type\n");
		return ret;
	}
	zinitix_err("tsp_vdd_supply_type: %d\n",pdata->tsp_supply_type);
			  ...
			  const struct i2c_device_id *i2c_id)
{
	...
	if (!ret && info->ts_pinctrl) {
		 ...
		 */
		ret = pinctrl_select_state(info->ts_pinctrl,info->pinctrl_state_active);
		if (ret < 0)
	...
	zinitix_err("KEY_WAKEUP/KEY_SLEEP capability register finish\n");
#endif
...
err_gpio_request:
	...
	if (info->ts_pinctrl) {
		if (IS_ERR_OR_NULL(info->pinctrl_state_release)) {
			...
			ret = pinctrl_select_state(info->ts_pinctrl,info->pinctrl_state_release);
			if (ret)
...
static int bt541_ts_remove(struct i2c_client *client)
{
	...
	if (info->ts_pinctrl) {
		if (IS_ERR_OR_NULL(info->pinctrl_state_release)) {
			...
			err = pinctrl_select_state(info->ts_pinctrl,info->pinctrl_state_release);
			if (err)
				zinitix_err("failed to select relase pinctrl state\n");
		}
	}
