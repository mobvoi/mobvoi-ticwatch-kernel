...
#else
		__pm_wakeup_event(&data->wakelock_read, 250); /* 250 msecs */
#endif
	}
	release_wakeup(data);
}
...
#else
	wakeup_source_init(&data->wakelock_read, "nanohub_wakelock_read");
#endif
...
#else
	wakeup_source_trash(&data->wakelock_read);
#endif
	vfree(buf);
fail_vma:
	if (own_iio_dev)
